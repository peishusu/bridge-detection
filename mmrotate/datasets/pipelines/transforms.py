...
class NormalizeImgFPN:
    ...
    def __call__(self, results):
        ...
        if isinstance(results['g_img_list'],list):
            for im in range(len(results['g_img_list'])):
                results['g_img_list'][im]=mmcv.imnormalize(results['g_img_list'][im],
...
class RandomFlipImgFPN:
    ...
    def ratio_flip(self, bboxes, img_shape, direction):
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            if isinstance(results['g_img_list'], list):
                for im in range(len(results['g_img_list'])):
                    results['g_img_list'][im]=mmcv.imflip(
                        results['g_img_list'][im], direction=results['flip_direction'])
            ...
            if isinstance(results['g_img_infos'], list):
                if results['g_img_list'] is not None:
                    if len( results['g_img_list'])>0:
                        for gimg in range(len(results['g_img_infos'])):
                            results['g_img_infos'][gimg]['gt_box']\
                                = self.bbox_flip(results['g_img_infos'][gimg]['gt_box'],
                                                results['img_shape'],
                                                results['flip_direction'])
                            results['g_img_infos'][gimg]['rel_x0y0x1y1']\
                                = self.ratio_flip(results['g_img_infos'][gimg]['rel_x0y0x1y1'],
                                                (1,1,1),results['flip_direction'])
