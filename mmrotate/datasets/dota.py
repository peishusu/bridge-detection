...
class DOTADataset(CustomDataset):
    ...
    def evaluate(self,
        ...
        if metric == 'mAP':
            ...
            print(f'scale_ranges:{scale_ranges}')
            print(f'self.CLASSES:{self.CLASSES}')
            print(f'logger:{logger}')

            mean_ap50, result50 = eval_rbbox_map(
                results,
                annotations,
                scale_ranges=scale_ranges,  # None
                iou_thr=iou_thr50,
                dataset=self.CLASSES,
                logger=logger,
                nproc=nproc)
            print('mean ap 50:',mean_ap50)
            # ap75
            mean_ap75, result75 = eval_rbbox_map(
                results,
                annotations,
                scale_ranges=scale_ranges,  # None
                iou_thr=iou_thr75,
        ...
        iou_thr_all = [0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95]
        # rangesbig 
        # ranges_huge=[(100,1000)]
        ranges_huge=[(0,50),(50, 200),(200,800),(800,20000)]
        print('---length ranges2---:',ranges_huge)          
        # print('mAP length>800:')
        eval_results = OrderedDict()
        assert isinstance(iou_thr_all, list)
        mean_aps_huge = []
        for iou_thr in iou_thr_all:
            # print_log(f'\n{"-" * 15}iou_thr: {iou_thr}{"-" * 15}')
            mean_ap_all, _ = eval_rbbox_map(
                results,
                annotations,
                scale_ranges=ranges_huge,
                iou_thr=iou_thr,
                dataset=self.CLASSES,
                logger='silent',
                nproc=nproc)
            mean_aps_huge.append(mean_ap_all)
            # eval_results_all_ranges[f'AP{int(iou_thr * 100):02d}'] = round(mean_ap_all, 3)
        print('all ap huge(50/200/800):', mean_aps_huge)  
        mAP_list_huge = []
        for i in range(len(ranges_huge)):
            mAP = 0
            for j in range(len(mean_aps_huge)):
                mAP += mean_aps_huge[j][i]
            mAP /= len(mean_aps_huge)
            mAP_list_huge.append(mAP)
        eval_results['mAP'] = mAP_list_huge
        print('mAP huge(50/200/800):',eval_results['mAP'])
    ...
    def merge_det(self, results, nproc=4):
        ...
        iou_thr=0.5
        merge_func = partial(_merge_func, CLASSES=self.CLASSES, iou_thr=iou_thr)
        print(f'DOTA merge iou thr:{iou_thr}')
