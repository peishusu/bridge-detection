...
def tpfp_default(det_bboxes,
    ...
    if gt_bboxes.shape[0] == 0:
        if area_ranges == [(None, None)]:
            ...
            # mmdet的写法如下
            det_areas = (
                det_bboxes[:, 2] - det_bboxes[:, 0] + 0) * (
                    det_bboxes[:, 3] - det_bboxes[:, 1] + 0)
    ...
    for k, (min_area, max_area) in enumerate(area_ranges):
        ...
        if min_area is None:
            ...
            gt_length = np.maximum(gt_w, gt_h)
            gt_area_ignore = (gt_length < min_area) | (gt_length >= max_area)
            # gt_areas = gt_w * gt_h
        ...
        for i in sort_inds:
            if ious_max[i] >= iou_thr:
                ...
                bbox = det_bboxes[i, :5]
                length = np.maximum(bbox[2],bbox[3])
                if length >= min_area and length < max_area:
                    fp[k, i] = 1
...
def eval_rbbox_map(det_results,
    ...
    # print(f'det_results[0]:{det_results[0]}')
    # print(f'annotations:{annotations}')
    # print(f'annotations[0]:{annotations[0]}')
    ...
    for i in range(num_classes):
        ...
        for cls_det in range(len(cls_dets)):
            tmp_det=np.array(cls_dets[cls_det])
            if tmp_det.shape[1]==5:  # 形状不符合(n,6),加一列
                add_=np.ones(tmp_det.shape[0])*0.5
                cls_dets[cls_det]=np.insert(tmp_det,tmp_det.shape[1],add_,axis=1)

        # compute tp and fp for each image with multiple processes
        # print(f'cls_dets:{cls_dets}')
        # print(f'cls_gts:{cls_gts}')
        # print(f'cls_gts_ignore:{cls_gts_ignore}')
        # print(f'iou_thr:{iou_thr}')
        # print(f'area_ranges:{area_ranges}')
        ...
        for _, bbox in enumerate(cls_gts):
            if area_ranges is None:
                ...
                # length
                long_side = np.maximum(bbox[:, 2], bbox[:, 3])
                for k, (min_length, max_length) in enumerate(area_ranges):
                    num_gts[k] += np.sum((long_side >= min_length)
                                         & (long_side < max_length))
        ...
        # print('clse_dets shape:',cls_dets.shape)
