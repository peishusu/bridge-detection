...
class RotatedBBoxHead(BaseModule):
    ...
    def _get_target_single(self, pos_bboxes, neg_bboxes, pos_gt_bboxes,
        ...
        if num_pos > 0:
            ...
            if self.use_spaw:
                # # SPAW
                gt_cx = pos_gt_bboxes[:,0]
                gt_cy = pos_gt_bboxes[:,1]
                gt_w = pos_gt_bboxes[:, 2]
                gt_h = pos_gt_bboxes[:, 3]
                

                angle=pos_gt_bboxes[:,-1]

                cos_angle = torch.cos(angle)
                sin_angle = torch.sin(angle)

                pos_xy=pos_bboxes[:,0:2]
                gt_xy=pos_gt_bboxes[:,0:2]
                v_d_xy = torch.sub(pos_xy, gt_xy)
                # dis = torch.zeros_like(num_pos).reshape(-1)
                delta_d = torch.sqrt(
                    (torch.pow(gt_xy[:, 0] - pos_xy[:, 0], 2)
                    + torch.pow(gt_xy[:, 1] - pos_xy[:, 1], 2))
                )

                w_dot_d = gt_w * (cos_angle * v_d_xy[:, 0] + sin_angle * v_d_xy[:, 1])
                h_dot_d = gt_h * (-sin_angle * v_d_xy[:, 0] + cos_angle * v_d_xy[:, 1])
                cos_a_wd = w_dot_d / (delta_d * gt_w)
                cos_a_hd = h_dot_d / (delta_d * gt_h)
                rela_w = torch.abs(delta_d * cos_a_wd)
                rela_h = torch.abs(delta_d * cos_a_hd)
                Qw = (rela_w * 2) / gt_w
                Qh = (rela_h * 2) / gt_h
                Qw[torch.isnan(Qw)] = 0.
                Qh[torch.isnan(Qh)] = 0.
                qua=(torch.log(Qw + 1) + 1) * (torch.log(Qh + 1) + 1)
                # tmp2=torch.ones_like(bbox_weights[:num_pos, :])
                # bbox_weights[:num_pos, :]=qua.unsqueeze(1)*tmp2
                gt_ratios=gt_h.unsqueeze(1)/gt_w.unsqueeze(1)
                factor=0.3
                # w= μ*(ln(ratio)+1)*ln[(Qw+1)+1]*ln[(Qh+1)+1]
                bbox_weights[:num_pos, :]=factor*qua.unsqueeze(1)*(torch.log(gt_ratios)+1).repeat(1,5)
