...
def list2tensor_(img_lists ,dim=0):
    ...
    if len(img_lists)>0:
        ...
        for img_list in img_lists:
            if img_list.shape[-1] == 5:
                # Replace tensor with a tensor of zeros with shape (n, 6)
                new_img_list = torch.zeros(*img_list.shape[:-1], 6, device=device)
            else:
                new_img_list = img_list.to(device)
            new_img_lists.append(new_img_list)
        inputs = torch.cat(new_img_lists, dim=dim)
    else:
        inputs = torch.tensor([])
