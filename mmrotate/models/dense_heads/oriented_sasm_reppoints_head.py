...
class OrientedSASMRepPointsHead(BaseDenseHead):
    ...
    def __init__(self,
                 ...
                 init_cfg=dict(
                     ...
                     override=dict(
                         ...
                         bias_prob=0.01)),
                 **kwargs):
        super(OrientedSASMRepPointsHead, self).__init__(init_cfg)
    ...
    def pointsets_quality_assessment(self, pts_features, cls_score,
        ...
        # print('pts_features:',pts_features.cpu())
        # print('pos_inds:',pos_inds.cpu())
...
# 采样,为init和refine stage选择合适的样本点
    def _point_target_single(self,
        ...
        if stage == 'init':
            ...
            # points_xy = points_center_pts(pos_proposals, y_first=True) # SASM的
        ...
        if unmap_outputs:
            ...
            # print('sam_weights shape:',sam_weights.shape)
    ...
    def get_targets(self,
        ...
        (all_labels, all_label_weights, all_bbox_gt, all_proposals,
         ...
         sampling_result,all_sam_weights # SASM
         ) = multi_apply(
        ...
        if stage == 'init':
            ...
            return (labels_list, label_weights_list, bbox_gt_list,
                    ...
                    num_total_neg, None, sam_weights_list)
    ...
    def loss(self,
        ...
        (*_, bbox_gt_list_init, candidate_list_init, bbox_weights_list_init,
         ...
         sam_weights_list_init) = cls_reg_targets_init  # 这里即为正样本数

        center_list, valid_flag_list = self.get_points(
            featmap_sizes, img_metas, device=device)
        pts_coordinate_preds_refine = self.offset_to_pts(
            center_list, pts_preds_refine)
        # 到这一步其实两个用来算损失的点集pts_coordinate_preds_init和pts_coordinate_preds_refine都已经得到了
        # 可能疑惑为什么还要算下面的?因为:虽然得到了所有点集,但是没有选择出(下方为refine阶段的)正负样本!!!

        # 这里的base_features和refine_points_features分别是什么??
        # 答：base_features就是经过backbone+neck后的基础特征
        #    而refine_points_features是由每个点预测的9个点位置在对应特征图上采样后的结果
        refine_points_features, = multi_apply(self.get_adaptive_points_feature,
                                              base_features,
                                              pts_coordinate_preds_refine,
                                              self.point_strides)

        # print('pts_coordinate_preds_refine in 1149:',pts_coordinate_preds_refine)
        # print('pts_preds_refine in 1150:',pts_preds_refine)
        # print('base_features[0] in 1150:',base_features[0])
        ...
        ] # 问题1. 得到的是什么？refine后点的“特征”？答：features_pts_refine(21824,9,256),是所有9个点在特征图上的对应值
        ...
        (labels_list, label_weights_list, bbox_gt_list_refine, _,
         ...
         sam_weights_list_refine) = cls_reg_targets_refine  # 为什么这里是refine??答:因为传入参数为refine
        ...
        if num_pos:
            ...
            # print('计算 loss lcs前的labels_weights:',labels_weight)
            # print('计算 loss lcs前的sam_weights_refine:',sam_weights_refine)
            losses_cls=self.loss_cls(
            ...
            # print('计算 loss box前的pos_bbox_weights_refine:',pos_bbox_weights_refine)
            # print('计算 loss box前的sam_weights_pos_refine:',sam_weights_pos_refine)
