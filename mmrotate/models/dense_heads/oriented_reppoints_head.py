...
class OrientedRepPointsHead(BaseDenseHead):
    ...
    def get_adaptive_points_feature(self, features, pt_locations, stride):
        ...
        h = features.shape[2] * stride  # 在原图上对应的跨度
        w = features.shape[3] * stride  # 在原图对应的跨度

        pt_locations = pt_locations.view(pt_locations.shape[0],
                                         pt_locations.shape[1], -1, 2).clone()
        pt_locations[..., 0] = pt_locations[..., 0] / (w / 2.) - 1  # 将所有点的x坐标归一化到(-1,1)
        pt_locations[..., 1] = pt_locations[..., 1] / (h / 2.) - 1  # 将所有点的y坐标归一化到(-1,1)

        batch_size = features.size(0)
        sampled_features = torch.zeros([
            pt_locations.shape[0],  # Batchsize
            features.size(1),       # Channel
            pt_locations.size(1),   # Number of point set
            pt_locations.size(2)    # number of points:9
    ...
    def loss(self,
        ...
        refine_points_features, = multi_apply(self.get_adaptive_points_feature,
                                              ...
                                              self.point_strides)
        features_pts_refine = levels_to_images(refine_points_features) # 得到每层特征图在原图上的位置
        features_pts_refine = [
    ...
    def _get_bboxes_single(self,
        ...
        # print('mlvl_scores[0:4]:',mlvl_scores[0:4])
        # print('mlvl_scores shape:',mlvl_scores.shape)
