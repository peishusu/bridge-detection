...
class OrientedSASMRepPointsHead(BaseDenseHead):
    ...
    def pointsets_quality_assessment(self, pts_features, cls_score,
        ...
        print('472 行pts_features的长度为:',len(pts_features))
...
# 采样,为init和refine stage选择合适的样本点
    def _point_target_single(self,
        ...
        # if self.version=='le135':
        #     device=get_device()
        #     Pi_4=math.pi/4
        #     cos_a=math.cos(angles+Pi_4)
        # if self.version=='oc':
        ...
        rela_w=torch.abs(torch.mul(dis,cos_a_wd).unsqueeze(1))
        rela_h=torch.abs(torch.mul(dis,cos_a_hd).unsqueeze(1))
        Qw=torch.div(torch.mul(rela_w,2),width).squeeze()
        Qh=torch.div(torch.mul(rela_h,2),height).squeeze()
        Qw[torch.isnan(Qw)] = 0.
        Qh[torch.isnan(Qh)] = 0.
        bef=label_weights[pos_inds]
        qua_weights=label_weights*torch.exp(Qw)*torch.exp(Qh)
        aft=qua_weights[pos_inds]
    ...
    def loss(self,
        ...
        refine_points_features, = multi_apply(self.get_adaptive_points_feature,
                                              ...
                                              self.point_strides)
        print('refine_points_features(1061行)的长度:',len(refine_points_features))
