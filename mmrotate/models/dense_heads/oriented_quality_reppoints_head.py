...
class OrientedQuaRepPointsHead(BaseDenseHead):
    ...
    def __init__(self,
        ...
        print('——————Oriented Quality RepPoints——————\n')
...
# 采样,为init和refine stage选择合适的样本点
    def _point_target_single(self,
        ...
        if len(pos_inds) > 0:
            ...
            if pos_weight <= 0:
                ...
                label_weights[pos_inds] = pos_weight  # 如果pos_weight不设为-1,而是正数则使用该正数
        if len(neg_inds) > 0:
    ...
    def loss(self,
        ...
        if num_pos:
            ...
            #     cls_scores,labels,labels_weight*sam_weights_refine*lvl_labels_weight,
            #     avg_factor=num_pos 
            # ) #SASM
            losses_cls=self.loss_cls(
            ...
            losses_pts_refine = self.loss_bbox_refine(
                ...
                pos_bbox_weights_refine*sam_weights_pos_refine*cls_weights_pos_refine) # SASM把原有权重乘以一个sam_weight
            ...
            loss_border_refine = self.loss_spatial_refine(
                ...
                pos_bbox_weights_refine*sam_weights_pos_refine*cls_weights_pos_refine,
                avg_factor=None)
